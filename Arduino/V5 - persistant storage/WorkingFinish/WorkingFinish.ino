...
bool RFID_LOOP() {
  ...
  Serial.println(F("RFID: Buzz start"));
  delay(interval);
  digitalWrite(BUZZER, LOW);
  Serial.println(F("RFID: Buzz stop"));
...
void SD_PRELOAD() {
  // Read the existing file to preload the card list and card count
  // This is necessary to prevent duplicate entries and to keep track of the number of cards accross power cycles
  const String filename = "/finish.csv";

  if (!SD.exists(filename)) {
    Serial.println(F("SD_PRELOAD: No existing file, skipping preload."));
    return;
  }
  ...
  if (!myFile) {
    Serial.println(F("SD_PRELOAD: Error opening file for preload."));
    digitalWrite(SD_CS, HIGH);
    return;
  }

  Serial.println(F("SD_PRELOAD: Preloading existing data from SD card..."));

  // Skip the header line
  String header = myFile.readStringUntil('\n');
  Serial.println(F("SD_PRELOAD: Header skipped."));
  ...
  while (myFile.available()) {
    ...
    if (line.length() > 0) {
      ...
      if (fifthComma == -1) {
        Serial.println(F("SD_PRELOAD: Malformed line, skipping: " + line));
        continue;
      }

      String date = line.substring(0, firstComma);
      String time = line.substring(firstComma + 1, secondComma);
      String latStr = line.substring(secondComma + 1, thirdComma);
      String lngStr = line.substring(thirdComma + 1, fourthComma);
      String id = line.substring(fourthComma + 1, fifthComma);

      // Add the ID to the card list if it's not already there
      if (!isCardInList(id)) {
        addCardToList(id);
        CardCount++;
        Serial.println(F("SD_PRELOAD: Added card ID from file: " + id));
      }
    }
  }
  Serial.println(F("SD_PRELOAD: Finished preloading data"));
  Serial.println(F("SD_PRELOAD: Total cards loaded: " + String(CardCount)));
  myFile.close();
  digitalWrite(SD_CS, HIGH);
